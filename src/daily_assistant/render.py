...
def render_digest_page(digest: list[DigestItem | dict[str, Any]], date: str) -> str:
    ...
    """
    html_content = f"<html><head><title>Daily Digest - {date}</title></head><body>"
    html_content += f"<h1>Daily Digest for {date}</h1>"

    for item in digest:
        normalized_item = _coerce_digest_item(item)
        html_content += f"<div class='digest-item'>"
        html_content += f"<h2>{normalized_item.headline}</h2>"
        html_content += f"<p>{normalized_item.summary}</p>"
        html_content += f"<p><strong>Why it matters:</strong> {normalized_item.why_it_matters}</p>"
        html_content += f"<p><strong>Category:</strong> {normalized_item.category}</p>"
        html_content += f"<p><strong>Articles:</strong> {'; '.join(normalized_item.article_urls)}</p>"
        html_content += "</div><hr>"
...
def render_index(archive_dates: list[str]) -> str:
    ...
    for date in archive_dates:
        html_content += f"<li><a href='digest_{date}.html'>{date}</a></li>"
    html_content += "</ul>"
    html_content += "</body></html>"
    return html_content