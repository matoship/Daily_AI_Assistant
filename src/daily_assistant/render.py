from typing import Any

from daily_assistant.models import DigestItem

import html

_STYLE = """
:root {
    color-scheme: light dark;
    --bg: #f7f7f8;
    --surface: #ffffff;
    --text: #1a1a1a;
    --muted: #5b5f66;
    --border: #e3e4e7;
    --accent: #2f5fd8;
    --callout-bg: #eef2fd;
}
@media (prefers-color-scheme: dark) {
    :root {
        --bg: #14151a;
        --surface: #1c1e24;
        --text: #eceef1;
        --muted: #9aa0aa;
        --border: #2c2f37;
        --accent: #7ea1ff;
        --callout-bg: #1f2740;
    }
}
* { box-sizing: border-box; }
body {
    margin: 0;
    padding: 2.5rem 1.25rem;
    background: var(--bg);
    color: var(--text);
    font-family: -apple-system, "Segoe UI", Roboto, Helvetica, Arial, sans-serif;
    line-height: 1.55;
}
main {
    max-width: 42rem;
    margin: 0 auto;
}
header { margin-bottom: 2rem; }
h1 { font-size: 1.5rem; margin: 0 0 0.25rem; }
.subtitle { color: var(--muted); font-size: 0.9rem; margin: 0; }
nav.back { margin-bottom: 1.5rem; }
nav.back a { color: var(--accent); text-decoration: none; font-size: 0.9rem; }
nav.back a:hover { text-decoration: underline; }
article.digest-item {
    background: var(--surface);
    border: 1px solid var(--border);
    border-radius: 10px;
    padding: 1.25rem 1.5rem;
    margin-bottom: 1.25rem;
}
.badge {
    display: inline-block;
    font-size: 0.72rem;
    font-weight: 600;
    letter-spacing: 0.03em;
    text-transform: uppercase;
    color: var(--accent);
    background: var(--callout-bg);
    border-radius: 999px;
    padding: 0.2rem 0.65rem;
    margin-bottom: 0.6rem;
}
article.digest-item h2 { font-size: 1.15rem; margin: 0 0 0.5rem; }
article.digest-item p { margin: 0 0 0.75rem; color: var(--text); }
.why-it-matters {
    background: var(--callout-bg);
    border-radius: 8px;
    padding: 0.75rem 1rem;
    font-size: 0.92rem;
}
.why-it-matters strong { color: var(--accent); }
.sources {
    margin-top: 0.75rem;
    padding-top: 0.75rem;
    border-top: 1px solid var(--border);
    font-size: 0.85rem;
    color: var(--muted);
}
.sources a { color: var(--muted); margin-right: 0.75rem; }
.sources a:hover { color: var(--accent); }
.empty {
    color: var(--muted);
    text-align: center;
    padding: 3rem 1rem;
}
ul.archive { list-style: none; margin: 0; padding: 0; }
ul.archive li {
    border: 1px solid var(--border);
    background: var(--surface);
    border-radius: 8px;
    margin-bottom: 0.6rem;
}
ul.archive a {
    display: block;
    padding: 0.85rem 1.1rem;
    color: var(--text);
    text-decoration: none;
    font-weight: 500;
}
ul.archive a:hover { color: var(--accent); }
"""


def _page(title: str, body: str) -> str:
    return (
        "<!doctype html><html lang='en'><head><meta charset='utf-8'>"
        "<meta name='viewport' content='width=device-width, initial-scale=1'>"
        f"<title>{html.escape(title)}</title><style>{_STYLE}</style></head>"
        f"<body><main>{body}</main></body></html>"
    )


def _coerce_digest_item(item: DigestItem | dict[str, Any]) -> DigestItem:
    if isinstance(item, DigestItem):
        return item
    if isinstance(item, dict):
        return DigestItem(**item)
    raise TypeError(f"Expected DigestItem or dict, got {type(item).__name__}")


def render_digest_page(digest: list[DigestItem | dict[str, Any]], date: str) -> str:
    """
    Render the digest page as an HTML string.

    Args:
        digest (list[DigestItem | dict[str, Any]]): A list of digest items to be displayed on the page.
        date (str): The date for which the digest is generated.

    Returns:
        str: An HTML string representing the digest page.
    """
    safe_date = html.escape(date)
    body = (
        "<nav class='back'><a href='index.html'>&larr; Archive</a></nav>"
        f"<header><h1>Daily Digest</h1><p class='subtitle'>{safe_date}</p></header>"
    )

    if not digest:
        body += "<p class='empty'>No relevant news today.</p>"
        return _page(f"Daily Digest - {date}", body)

    for item in digest:
        normalized_item = _coerce_digest_item(item)
        sources = "".join(
            f"<a href='{html.escape(url)}'>source {i}</a>"
            for i, url in enumerate(normalized_item.article_urls, start=1)
        )
        body += (
            "<article class='digest-item'>"
            f"<span class='badge'>{html.escape(normalized_item.category)}</span>"
            f"<h2>{html.escape(normalized_item.headline)}</h2>"
            f"<p>{html.escape(normalized_item.summary)}</p>"
            "<p class='why-it-matters'><strong>Why it matters:</strong> "
            f"{html.escape(normalized_item.why_it_matters)}</p>"
            f"<div class='sources'>{sources}</div>"
            "</article>"
        )

    return _page(f"Daily Digest - {date}", body)


def render_index(archive_dates: list[str]) -> str:
    """
    Render the index page as an HTML string.

    Args:
        archive_dates (list[str]): A list of dates for which digests are available.

    Returns:
        str: An HTML string representing the index page.
    """
    body = "<header><h1>Daily Digest Archive</h1></header>"

    if not archive_dates:
        body += "<p class='empty'>No digests yet.</p>"
        return _page("Daily Digest Archive", body)

    items = "".join(
        f"<li><a href='digest_{html.escape(date)}.html'>{html.escape(date)}</a></li>"
        for date in archive_dates
    )
    body += f"<ul class='archive'>{items}</ul>"
    return _page("Daily Digest Archive", body)
